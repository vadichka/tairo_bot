...
# ========== ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ ДЛЯ ОБРАБОТКИ АЛЬБОМОВ ==========
async def process_album(message: types.Message, state: FSMContext, target: int, step_name: str):
    """Универсальная функция для обработки альбомов"""
    ...
    if media_group_id:
        ...
        # Добавляем фото (берём самое большое качество)
        temp_albums[media_group_id]["photos"].append(message.photo[-1].file_id)
        
        # Ждём 2 секунды, чтобы собрать все фото альбома
        await asyncio.sleep(2)
        
        # Если альбом ещё не обработан
        if media_group_id in temp_albums:
            album_data = temp_albums.pop(media_group_id)
            photos = list(dict.fromkeys(album_data["photos"]))  # Убираем дубликаты
...
async def get_badge(message: types.Message, state: FSMContext):
    ...
    await message.answer("💰 Шаг 4 из 4: Валюта\n\nВыбери валюту:", reply_markup=currency_kb)
...
async def choose_model_specs(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "📸 Пришли 4 фото авто с разных ракурсов\n\nЭто нужно что б мы могли максимально точно сделать модельку твоего авто",
        reply_markup=back_button
    )
    await callback.answer()
